...
homepageController.getAllUsers = async (req, res, next) => {
  try {
    ...
    const string = `SELECT user_id, firstName, lastName, bio, subject, skillLevel, email FROM users WHERE user_id != ${user_id} `;
  ...
  try {
    ...
    if (response.rowCount === 0) {
      next({
        ...
        status: 400,
        message: {
          err: 'An error occurred in createRequest middleware,user has been requested',
        },
      });
    }
    return next();
...
homepageController.displayRequest = async (req, res, next) => {
  try {
    ...
    res.locals.users = {
      wantToWorkWith: response2.rows,
      wantsToWorkWithYOU: response1.rows,
    };
    return next();
  } catch (error) {
    next({
